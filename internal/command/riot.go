...
func RiotGroup(api *riot.Client, store *storage.Client) Factory {
	return func() (*discordgo.ApplicationCommand, Handler) {
		...
		var riotCommand = &discordgo.ApplicationCommand{
			Name:        "lol",
		...
		return riotCommand, func(s *discordgo.Session, i *discordgo.InteractionCreate) {
			options := i.ApplicationCommandData().Options
			log.Debugf("Responding to lol.%s", options[0].Name)
			switch options[0].Name {
			case "link":
				linkHandler(s, i, api, store)
			default:
...
func linkHandler(s *discordgo.Session, i *discordgo.InteractionCreate, api *riot.Client, store *storage.Client) {
	...
	go func() {
		summoner, err := api.SummonerByName(ctx, region, summonerName)
		if err != nil {
		...
		err = store.InsertDiscordSummoner(ctx, usr.ID, summoner)
		if err != nil {
